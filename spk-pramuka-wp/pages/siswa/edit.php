...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $errors = require_fields($_POST, ['kode_siswa' => 'Kode siswa', 'nama_siswa' => 'Nama siswa', 'nis' => 'NIS', 'no_handphone' => 'No handphone', 'jenis_kelamin' => 'Jenis kelamin']);
    if (!$errors) {
        $stmt = $pdo->prepare('UPDATE siswa SET kode_siswa=?, nama_siswa=?, nis=?, no_handphone=?, jenis_kelamin=? WHERE id_siswa=?');
        $stmt->execute([trim($_POST['kode_siswa']), trim($_POST['nama_siswa']), trim($_POST['nis']), trim($_POST['no_handphone']), $_POST['jenis_kelamin'], $id]);
